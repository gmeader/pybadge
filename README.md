...
* pygamer/MIDIcommander.py - A "patch librarian" that stores sysex files (or files containing any MIDI command byte strings) on SDcard; plays them out the MIDI Featherwing port. Uses joystick to navigate folders of files from the SDcard on teh LCD display. 
